...
class RewritingSumHelper:
    ...
    @classmethod
    def rs_sum_generate_alldiff_rules_helper(cls, rule_head_name, sum, elements, str_type, str_id, variable_dependencies, aggregate_mode, cur_variable_dependencies, always_add_variable_dependencies, skolem_constants):

        rules_strings = []

        for secondary_guard_value in range(1, sum + 1):
            tmp_rules_strings = RSHelper._rs_count_generate_count_rule(rule_head_name, secondary_guard_value, elements, str_type, str_id, variable_dependencies, aggregate_mode, cur_variable_dependencies, always_add_variable_dependencies, skolem_constants, total_count=sum)
    ...
    @classmethod
    def rs_plus_star_sum_generate_alldiff_rules_helper(cls, rule_head_name, sum, elements, str_type, str_id, variable_dependencies, aggregate_mode, cur_variable_dependencies, always_add_variable_dependencies):

        rules_strings = []
        rules_head_strings = []

        for secondary_guard_value in range(1, sum + 1):
            tmp_rules_strings, tmp_rules_head_strings = RSPlusStarHelper._rs_plus_star_generate_all_diff_rules(rule_head_name, secondary_guard_value, elements, str_type, str_id, variable_dependencies, aggregate_mode, cur_variable_dependencies, always_add_variable_dependencies, sum)
