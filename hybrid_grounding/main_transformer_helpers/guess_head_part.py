...
class GuessHeadPart:
    ...
    def do_ground_guess(self, new_head_name):
        ...
        body_dom_dict = {}
        if self.current_rule in self.rule_strongly_connected_components and self.cyclic_strategy == CyclicStrategy.SHARED_CYCLE_BODY_PREDICATES:
        ...
        for body_combination in body_combinations:
            ...
            possible_head_guesses = self.generate_grounded_head_guesses(new_head_name, body_dom_list_lookup, body_combination)

            if self.current_rule in self.rule_strongly_connected_components and self.cyclic_strategy == CyclicStrategy.SHARED_CYCLE_BODY_PREDICATES:
                self.print_grounded_head_guess_for_shared_cycle_body_predicates(body_dom_list_lookup, body_combination, possible_head_guesses)
    ...
    def print_grounded_head_guess_for_shared_cycle_body_predicates(self, body_dom_list_lookup, body_combination, possible_head_guesses):
        ...
        for predicate in self.rule_strongly_connected_components[self.current_rule]:
            ...
            for argument in [str(argument) for argument in predicate.arguments]:
                ...
                predicate_arguments.append(body_combination[combination_argument_position])
        ...
        self.printer.custom_print(f"{{{';'.join(possible_head_guesses)}}} {parsed_grounded_predicates}")

    def generate_grounded_head_guesses(self, new_head_name, body_dom_list_lookup, body_combination):
        ...
        for head_combination in head_combinations:
            ...
            for argument in [str(argument) for argument in self.rule_head.arguments]:
                if argument not in head_dom_list_lookup and argument not in body_dom_list_lookup:
                    print("FATAL in guess head, could not find argument in current combination!")
                    raise(Exception)

                if argument in head_dom_list_lookup: 
                    combination_argument_position = head_dom_list_lookup[argument]
                    current_head_guess_arguments.append(head_combination[combination_argument_position])
                else:
                    combination_argument_position = body_dom_list_lookup[argument]
                    current_head_guess_arguments.append(body_combination[combination_argument_position])

            if len(current_head_guess_arguments) > 0:
                parsed_current_head_guess_arguments = f"({','.join(current_head_guess_arguments)})"
            else:
                parsed_current_head_guess_arguments = ""

            possible_head_guesses.append(f"{new_head_name}{parsed_current_head_guess_arguments}")

            self.printer.custom_print(f"{self.rule_head.name}{parsed_current_head_guess_arguments} :- {new_head_name}{parsed_current_head_guess_arguments}.")
    ...
    def non_ground_guess(self, new_head_name, new_head):
        ...
        if self.current_rule in self.rule_strongly_connected_components and self.cyclic_strategy == CyclicStrategy.SHARED_CYCLE_BODY_PREDICATES:
            string_preds = ",".join([str(predicate) for predicate in self.rule_strongly_connected_components[self.current_rule]])
